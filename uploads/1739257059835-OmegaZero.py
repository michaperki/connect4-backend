...
class HelperFunctions:
    ...
    def place(choice: int, board, player: int):
        """Return a new board with the token placed in column `choice`.
    ...
    def check_win(board):
        """
        Uses convolution to check if either player has 4 in a row.
        Returns:
          1 if player 1 wins,
         -1 if player -1 wins,
          0 if draw,
...
class OmegaZero:
    def __init__(self):
        ...
        self.TIME_LIMIT = 295  # Total time limit in seconds (e.g. 5 minutes minus a buffer)
    ...
    def get_move(self, board, player=1, **kwargs):
        """Main function: returns the chosen column for the move."""
        self.player = player

        # Detect new game: if the current board has fewer tokens than the previous one.
...
###############################
# Main: Parse args and output move
###############################
if __name__ == '__main__':
    ...
    # If no valid move is found, print -1
