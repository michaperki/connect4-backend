...
class HelperFunctions:
    ...
    def place(choice: int, board, player: int):
        """Return a new board with the token dropped into column `choice`.
    ...
    def get_valid_moves(board):
        """Return a list of column indices where the top cell is 0."""
        return np.where(board[0] == 0)[0].tolist()

    @staticmethod
    def check_win(board):
        """Uses convolution to check for a win.
           Returns 1 if player 1 wins, -1 if player -1 wins,
           0 if draw, or None if nonterminal.
        """
...
class OmegaZero:
    def __init__(self):
        ...
        self.TIME_LIMIT = 295  # total game time in seconds
        self.start_time = None
        self.max_depth = 1
        self.player = None
        self.time_per_move = 10  # initial allocation per move in seconds
        self.total_moves = 0
    ...
    def get_move(self, board, player=1, **kwargs):
        self.player = player
        # Detect new game if board has fewer tokens than before
        ...
        try:
            ...
            # Force a conservative per-move time (e.g. 2 seconds)
            effective_time = min(self.time_per_move, 2)
            ...
            print(best_move if best_move is not None else -1, flush=True)
            return best_move

        except Exception as e:
            logging.error(f"Unexpected error in get_move: {e}")
            valid_moves = HelperFunctions.get_valid_moves(board)
            fallback = valid_moves[0] if valid_moves else -1
